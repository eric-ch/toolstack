...
let add_device ~xs device backend_list frontend_list =
	...
	Xs.transaction xs (fun t ->
		...
		(* tomaszw: XC-7081: contrary to above, clear the 'hotplug path' because
                   we in xenclient don't record anything there and it prevents proper xs permissions being
                   reestablished on reboot of domain holding driver backends *)
...
let mount ty readonly path keydir =
	...
	let out, log =
		try Forkhelpers.execute_command_get_output ~withpath:true ~env "/usr/sbin/tap-ctl"
			    (List.concat [["create"];
					  (if readonly then ["-R"] else []);
					  ["-a"; sprintf "%s:%s" ty path]
					 ])
...
(** Request either a clean or hard shutdown
	Return true if we need to wait the shutdown
	TODO: this code needs to be common with the other devices
  *)
let request_shutdown ~xs (x: device) (force: bool) =
	...
	Xs.transaction xs (fun t ->
		...
		(* I think we need to use Xenbus.Closed as xl does but in this case,
		 * when the backend state is still in InitWait state, closing will
		 * never occured. (Test copied from vif request_closure)
		 *)
...
(** Return the event to wait for when the shutdown has completed
    The function return a string Watch.t to avoid problem in other parts of xenvm
 *)
let shutdown_done ~xs (x: device): string Watch.t =
	let backend_path = backend_path_of_device ~xs x in
	let state_path = backend_path ^ "/state" in
	let closed = Xenbus.string_of Xenbus.Closed in
	Watch.map (fun () -> "") (Watch.value_to_become state_path closed)

let hard_shutdown_request ~xs (x: device) = ignore (request_shutdown ~xs x true)
...
let hard_shutdown ~xs (x: device) =
	...
	if exists then (
		if request_shutdown ~xs x true (* force *) then (
                    ...
                    debug "Device.Vbd.hard_shutdown about to away frontend";
                    let frontend_path = frontend_path_of_device ~xs x in
                    xs.Xs.rm frontend_path;

                    ignore(Watch.wait_for ~xs (unplug_watch ~xs x));

                    debug "Device.Vbd.hard_shutdown about to blow away backend";
                    Generic.safe_rm ~xs backend_path;
		);
		Generic.rm_device_state ~xs x);
...
let add_struct ~xc ~xs ~hvm ?(use_stubdom=false) ?(protocol=Protocol_Native) ?(backend_domid=0) diskinfo domid  =
	...
	*)
	...
	let dm_physpath = if use_stubdom then ("/dev/" ^ diskinfo.virtpath) else diskinfo.physpath in
	...
	| File ->
		...
		let backend_ty, backend = backend_blk "file" dm_physpath in
	...
	| Phys ->
		...
		backend_blk "raw" dm_physpath
	| Qcow | Vhd | Aio ->
		backend_tap (string_of_physty diskinfo.phystype) dm_physpath
		in
	...
	Hashtbl.add_list front_tbl [
		...
		"backend-uuid", Uuid.to_string (get_uuid xc backend_domid);
	...
	Hashtbl.add_list back_tbl [
		...
		"type", if use_stubdom then "phy" else backendty_of_physty diskinfo.phystype;
...
(* for compat *)
let add ~xc ~xs ~hvm ~mode ~virtpath ~phystype ~physpath ~dev_type ~unpluggable ~diskinfo_pt
        ?use_stubdom ?protocol ?extra_backend_keys ?backend_domid domid =
	let diskinfo = {
	...
	add_struct ~xc ~xs ~hvm ?use_stubdom ?protocol ?backend_domid diskinfo domid
...
let get_loop_device ~xs ~virtpath domid =
	let devid = device_number virtpath in
	let backend = { domid = 0; kind = Vbd; devid = devid } in
	let path = sprintf "%s/loop-device" (backend_path ~xs backend domid) in
	try
		let r = xs.Xs.read path in
		Some r
	with _ ->
		None
...
let common_vif_clean_shutdown ~xs (x: device) =
	let kind_str = string_of_kind x.frontend.kind in
	debug "Device.%s.clean_shutdown %s" kind_str (string_of_device x);

	let back = have_backend ~xs x in
	if back then (
		common_vif_request_closure ~xs x;
		match Watch.wait_for ~xs (Watch.any_of [ `OK, common_vif_unplug_watch ~xs x;
							 `Failed, common_vif_error_watch ~xs x ]) with
		| `OK, _ ->
			(* Delete the trees (otherwise attempting to plug the device in again doesn't
			   work. This also clears any stale error nodes. *)
			Generic.rm_device_state ~xs x
		| `Failed, error ->
			debug "Device.%s.shutdown_common: read an error: %s" kind_str error;
			raise (Device_error (x, error))
	) else Generic.rm_device_state ~xs x

let common_vif_hard_shutdown ~xs (x: device) =
	let kind_str = string_of_kind x.frontend.kind in
	debug "Device.%s.hard_shutdown %s" kind_str (string_of_device x);
	...
	(* blow away the frontend *)
	debug "Device.%s.hard_shutdown about to blow away frontend" kind_str;
	let frontend_path = frontend_path_of_device ~xs x in
	xs.Xs.rm frontend_path;

	if back then
		ignore(Watch.wait_for ~xs (common_vif_unplug_watch ~xs x));

	let backend_path = backend_path_of_device ~xs x in
	debug "Device.%s.hard_shutdown about to blow away backend" kind_str;
	Generic.safe_rm ~xs backend_path;

	(* blow the udev path *)
	Generic.safe_rm ~xs (udev_path_of_device ~xs x)
...
let get_backend_dev ~xs (x: device) =
	if Hotplug.use_new_mechanism then (
		sprintf "vif%d.%d" (x.frontend.domid) (x.backend.domid)
	) else (try
...
let add_struct ~xc ~xs ?(protocol=Protocol_Native) ?(backend_domid=0) vifinfo domid =
	...
	let back_options =
		...
		in
	let network = match Netman.networkid_of_netty vifinfo.netty with
		| Some id -> ["bridge", id]
		| _ -> [] in
	let back = [
		"frontend-id", sprintf "%u" domid;
...
let add ~xc ~xs ~devid ~netty ~mac ?mtu ?(rate=None) ?(protocol=Protocol_Native) ?(backend_domid=0) domid =
	let vifinfo = { vifid = devid; netty = netty; mac = mac; mtu = mtu; rate = rate; } in
	add_struct ~xc ~xs ~protocol ~backend_domid vifinfo domid

let clean_shutdown ~xs (x: device) = common_vif_clean_shutdown ~xs x
let hard_shutdown ~xs (x: device) = common_vif_hard_shutdown ~xs x
...
let add ~xc ~xs ~devid ~netty ~mac ?mtu ?(rate=None) ?(protocol=Protocol_Native) ?(backend_domid=0) domid =
	...
	let network = match Netman.networkid_of_netty netty with
		| Some id -> ["bridge", id]
		| _ -> [] in
	let back = [
		"frontend-id", sprintf "%u" domid;
...
let read_dev_class dev =
  let devstr = string_of_desc dev.desc in
  let p = "/sys/bus/pci/drivers/pciback/" ^ devstr ^ "/class" in
  let str = String.strip (fun c -> c = '\n') (read_whole_file_to_string p) in
  int_of_string str
...
let add_noexn ~xc ~xs ~hvm ?(assign=true) ?(pvpci=true) ?(flr=false) ?(permissive=false) pcidevs_list domid devid =
	...
	if flr then (List.iter (fun (dev, resources) -> 
		let cls = read_dev_class dev in
		if cls <> 0x20000 && cls <> 0x28000 then do_flr dev
	) pcidevs);

	(* reread host pci info after FLR - it might've changed *)
	let pcidevs = List.map (fun info ->
		let (irq, memaddr, driver) = get_from_system info.desc in
		info, { irq = irq; memaddr = memaddr; driver = driver; }
	) pcidevs_list in
	...
	List.iter (fun (dev, resources) ->
		...
		if resources.irq > 0 then (
			let irq = Xc.physdev_map_pirq xc domid resources.irq in
	...
	let others = if flr then [("flr", "1")] else [] in
	...
	(* if pvpci=false, do not initialise the pciback/front driver pair (leave state node out)
         * required for stubdoms as the pvdriver shall be initialised for the stubdom but not for guest
	 * (it does not allow for multiple instances for same device)
	 * we still make entries in xenstore to aid with device cleanup.
	 *)
	let backendlist = [
		"frontend-id", sprintf "%u" domid;
		"online", "1";
		...
		"power_mgmt", string_of_int (power_mgmt);
	] @ if pvpci then [ "state", string_of_int (Xenbus.int_of Xenbus.Initialising); ] else []
	  and frontendlist = [
		"backend-id", "0";
		"state", string_of_int (Xenbus.int_of Xenbus.Initialising);
	] in
...
let release ~xc ~xs ~hvm pcidevs domid devid =
	...
	let report_errors f =
		try f() with exn -> debug "exception %s" (Printexc.to_string exn) in
	let free domid dev resources deassign =
		debug "Device.Pci.release domid=%d %d %d %d %d" domid dev.desc.domain dev.desc.bus dev.desc.slot dev.desc.func;
		if deassign then (
			debug "deassign device domid=%d %d %d %d %d" domid dev.desc.domain dev.desc.bus dev.desc.slot dev.desc.func;
			report_errors (fun _ -> Xc.domain_deassign_device xc domid (dev.desc.domain, dev.desc.bus, dev.desc.slot, dev.desc.func))
		);
		if resources.irq > 0 then (
			debug "unmap pirq";
			(* this tends to throw EINVAL if pirq is already freed, hence the catch *)
			report_errors (fun _ -> Xc.physdev_unmap_pirq xc domid resources.irq);
			debug "remove irq permission";
...
let clean_shutdown ~xs (x: device) =
	...
	Xc.with_intf (fun xc ->
		...
		with exn ->
			debug "clean_shutdown: Caught exception %s while destroying device %s"
				(Printexc.to_string exn) (string_of_device x);
	);
	()
...
let add ~xc ~xs ~hvm domid =
	debug "Device.Vsnd.add %d" domid;

	let frontend = { domid = domid; kind = Vsnd; devid = 0 } in
	let backend = { domid = 0; kind = Vsnd; devid = 0 } in
	let device = { backend = backend; frontend = frontend } in
...
let hard_shutdown ~xs (x: device) =
	debug "Device.Vsnd.hard_shutdown %s" (string_of_device x);
	()

let clean_shutdown ~xs (x: device) =
	debug "Device.Vsnd.clean_shutdown %s" (string_of_device x);
	()
...
let add ~xc ~xs ~hvm ~instance domid =
	debug "Device.Vtpm.add %d %d" instance domid;
	let frontend = { domid = domid; kind = Vtpm; devid = 0 } in
	let backend  = { domid = 0    ; kind = Vtpm; devid = 0 } in
	let device   = { backend = backend; frontend = frontend } in
	let dev_uuid' = read_whole_file_to_string "/proc/sys/kernel/random/uuid" in
	let dev_uuid  = String.sub dev_uuid' 0 (String.length dev_uuid' - 1) in
	let back = [
		"frontend-id", sprintf "%u" domid;
		"online", "1";
		"state", string_of_int (Xenbus.int_of Xenbus.Initialising);
		"pref_instance", string_of_int instance;
		"resume", "False";
		"uuid", dev_uuid;
		"domain", "vtpm-domain"
	] in
	...
	Generic.add_device ~xs device back front;
	let env = [| "XENBUS_PATH=" ^ backend_path_of_device ~xs device |] in
	let _ =
		try Forkhelpers.execute_command_get_output ~withpath:true ~env "/etc/xen/scripts/vtpm"
			    [ "add" ]
		with Forkhelpers.Spawn_internal_error (log, output, status) ->
			let s = sprintf "output=%S status=%s" output (string_of_unix_process status) in
			debug "Device.Vtpm.add vtpm script failed %s" s;
			("","")
			(* raise (Device_error (device, s)) *)
	in()
end
...
let add ~xs ~hvm ?(protocol=Protocol_Native) ?(backend_domid=0) ~output ~consback ~devid domid =
	...
	let special =
		if not hvm && consback = Ioemu && devid = 0 then (
			let port = xs.Xs.read (sprintf "%s/console/port" (xs.Xs.getdomainpath domid)) in
			let ringref = xs.Xs.read (sprintf "%s/console/ring-ref" (xs.Xs.getdomainpath domid)) in
			Some (port, ringref)
		) else None in
